...
export namespace nen
{
	...
	public:
		NSocket() noexcept
			: sock(checkInitSock()), ssl(initSSL(sock)) { initedCount++; }
		...
		~NSocket() noexcept
		{
			close(); clean(sock, ssl); initedCount--;
			if (!initedCount) { WSACleanup(); }
		}

		template<typename S, typename I>
		decltype(auto) req(S&& symbol, I&& interval) noexcept
		{ sendReq(ssl, reqKlines(std::forward<S>(symbol), std::forward<I>(interval))); return receive(ssl); }
